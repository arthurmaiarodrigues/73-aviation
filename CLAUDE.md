...
- `schema_v24_combustivel_so_na_compra.sql` (25/09): combustível não é mais cobrado duas vezes. `v_extrato_socio` perdeu a linha COMBUSTÍVEL (saldo de litros valorado): o combustível é pago **na compra do tanque**, rateada pelos litros que cada sócio retirou desde a compra anterior (`litros_tanque_por_socio`, schema_v9). O saldo de litros continua em Extratos e Fechamento como controle, sem valor.
...
- `schema_v25_fundo_zerado.sql` (25/09): fundo de reserva **zerado até aqui** (os R$ 150/h de maio a setembro saíram dos extratos; saldo do fundo = R$ 0,00). Valor por hora sem vigência e `aeronaves.fundo_reserva_por_hora = 0`; `provisionar_fundo_reserva` não cria linha quando o valor é 0. Os sócios vão se reunir para decidir o valor — quando decidirem, lançar em /fundo com o mês a partir do qual vale (reprovisiona os meses abertos). Telas mostram "a definir" no lugar de R$ 0,00.
