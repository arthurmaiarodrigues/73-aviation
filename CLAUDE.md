...
- Conferência em lote (22/09): card `ConferirLote` no topo de /reembolsos (admin) — lista os PENDENTES com checkbox (todos marcados), mostra a divisão que o piloto marcou, seletor "Manter a divisão do piloto / Todos iguais / Todos pelas horas / Só FULANO" e botão "Confirmar N · R$ X" (ação `confirmarVarios`, um `confirmar_reembolso` por item, um push só no fim, `router.refresh()`). O seletor por linha (`ConferirReembolso`) foi removido.
