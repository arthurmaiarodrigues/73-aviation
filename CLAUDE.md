...
- `schema_v12_pernas.sql`: `voos.horimetro_pernas numeric[]` (leitura no pouso de cada escala) e `pernas_concluidas`. Registrar voo em 3 toques: foto → "Para onde?" com **Ida e volta** marcado (destino vira 1ª escala, destino final = origem) → Decolei; resto em "mais detalhes". Ficha do voo: `FormularioPerna` (`registrarPerna`) enquanto `pernas_concluidas < escalas.length`, depois `FormularioPouso` fecha (horas de cada perna pela diferença dos horímetros; `ListaEscalas` não aparece quando já há perna pousada). Início: "Perna N de M" e botão "Pousei em X".
